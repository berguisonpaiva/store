...
const ERROR_MAP: Record<string, (code: string) => HttpException> = {
  ...
  CANNOT_DEACTIVATE_SELF: (c) => new UnprocessableEntityException(c),
  ...
  VENDA_PENDENTE_NO_FECHAMENTO: (c) => new UnprocessableEntityException(c),
  NAO_E_DONO_DO_CAIXA: (c) => new ForbiddenException(c),
  ACESSO_NEGADO: (c) => new ForbiddenException(c),
  VALOR_INVALIDO: (c) => new UnprocessableEntityException(c),
  // Vendas (PDV sales)
  ...
  INVALID_PRICE: (c) => new UnprocessableEntityException(c),
