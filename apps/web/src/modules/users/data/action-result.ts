...
/**
 * Discriminated result returned by every users Server Action. Success carries
 * the backend entity; failure carries the stable domain `code` (e.g.
 * `EMAIL_ALREADY_IN_USE`, `CANNOT_DEACTIVATE_SELF`) and HTTP `status`, which the
 * client maps to a field error or a toast.
 */
