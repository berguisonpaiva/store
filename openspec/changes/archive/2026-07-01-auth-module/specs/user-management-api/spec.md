...
**Reason**: The `change-password` use case is removed from scope for the ADMIN/OPERADOR MVP.
**Migration**: `PATCH /api/users/:id/password`, its HTTP DTO, and the `INVALID_CURRENT_PASSWORD` mapping are removed. No self-service password endpoint remains.

### Requirement: Initial MASTER seed

**Reason**: The `MASTER` role is removed (RN01); the bootstrap user is now an `ADMIN`.
**Migration**: Replaced by "Initial ADMIN seed" above. Any existing `MASTER` rows are migrated to `ADMIN` via the schema migration.
