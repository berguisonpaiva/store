...
The system SHALL expose role-protected endpoints to create, edit, activate, deactivate, list (paginated, name search, category/status filters), and fetch products, delegating to the domain use cases.

#### Scenario: Create product

- **WHEN** an authorized user POSTs a valid product with at least one variation to `POST /api/products`
...
#### Scenario: Product without variation mapped to 400

- **WHEN** a create returns `PRODUCT_MUST_HAVE_VARIATION`
- **THEN** the endpoint responds 400 Bad Request (RF-CAT-02)
