...
The system SHALL provide a `consultar-saldo` query that returns, for a variation, its `saldoAtual` and its **available balance** computed as `saldoAtual − quantidadeReservada`. The query reads through the `EstoqueQuery` port and MUST NOT expose the internal ledger entity.

#### Scenario: Returns current and available balance

- **WHEN** `consultar-saldo` runs for an existing variation
- **THEN** it returns `saldoAtual` and `saldoDisponivel = saldoAtual − quantidadeReservada` (RF-EST-06)
