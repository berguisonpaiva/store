...
Important: the skill may keep internal test templates under `.Codex/skills/module-aggregate/assets/**`, but the generated test files must always be created at the module root under `modules/<module>/test/**`, never inside `modules/<module>/src/**`.
...
```bash
node .Codex/skills/module-aggregate/scripts/create-aggregate.js --module auth --aggregate user --mode crud
```
...
- Use only the resources contained in `.Codex/skills/module-aggregate`.
