...
2. Executar o script da skill:
   - `node .Codex/skills/config-prisma/scripts/init-prisma-backend.js --dry-run`
   - `node .Codex/skills/config-prisma/scripts/init-prisma-backend.js --apply --install`
   - `<pm> run prisma:generate` no workspace do backend (obrigatório antes de qualquer build; `<pm>` = package manager detectado)
3. Incluir arquivos Prisma por módulo (repetível):
   - `node .Codex/skills/config-prisma/scripts/init-prisma-backend.js --apply --module auth --module product`
4. Subir banco com Docker Compose do backend:
   - `<pm> run db:start` (ou `<pm> --workspace apps/backend run db:start` a partir da raiz, conforme o package manager)
5. Validar Prisma:
   - `<pm> run prisma:generate`
6. Rodar ciclo completo de banco (desenvolvimento local):
   - executar os scripts locais: `prisma:migrate:dev`, `prisma:generate` e `prisma:seed` com o package manager detectado.
...
- Scripts de banco/Prisma (`db:start`, `db:stop`, `db:logs`, `prisma:generate`, `prisma:migrate:*`, `prisma:seed`, `prisma:studio`) compatíveis com Bun/pnpm/npm.
- Script de desenvolvimento na raiz preparado para subir o banco automaticamente antes do Turbo (`<pm> run dev` -> `db:start` + `turbo run dev`).
- Remoção de script legado `prisma:cid` quando presente.
- `apps/backend/prisma.config.ts` com seed adaptado ao package manager detectado (`bunx tsx`, `pnpm exec tsx` ou `npx tsx`).
...
- Usar `../skills-standards.md` para resolver Bun/pnpm/npm; não assumir `npm`.
