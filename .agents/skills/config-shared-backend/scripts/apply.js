...
function resolvePackageManager() {
  ...
  if (fs.existsSync(packagePath)) {
    try {
      ...
      if (['bun', 'pnpm', 'yarn', 'npm'].includes(packageManager)) return packageManager
    } catch {}
  }
...
function addCommand(packageManager, deps) {
  if (packageManager === 'bun') return `bun add ${deps.join(' ')}`
  if (packageManager === 'pnpm') return `pnpm add ${deps.join(' ')}`
  if (packageManager === 'yarn') return `yarn add ${deps.join(' ')}`
  return `npm install ${deps.join(' ')}`
}

function runScriptCommand(packageManager, script, workspace) {
  if (packageManager === 'bun') return `bun --cwd ${workspace} run ${script}`
...
function ensureDependencies() {
  ...
  if (missing.length > 0 || missingTypes.length > 0) {
    ...
    execSync(addCommand(packageManager, allMissing), { cwd: path.dirname(backendPkg), stdio: 'inherit' })
  } else {
...
function buildSharedPackage() {
  ...
  execSync(runScriptCommand(resolvePackageManager(), 'build', 'packages/shared'), { cwd: ROOT, stdio: 'inherit' })
