...
/// Activates a variation on an existing product (RF-CAT-06).
export class ActivateVariation implements UseCase<SetVariationActiveInputDTO, ProductDTO> {
  ...
  async execute(input: SetVariationActiveInputDTO): Promise<Result<ProductDTO>> {
    ...
    const activated = found.instance.activateVariation(input.variationId)
