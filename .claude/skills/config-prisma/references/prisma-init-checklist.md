...
- Runtime/package manager do projeto instalado (Bun, pnpm ou npm)
...
```bash
node .claude/skills/config-prisma/scripts/init-prisma-backend.js --apply --module auth --module stock --module billing
```
...
- `--install`: roda install com o package manager detectado pelo projeto (Bun, pnpm, yarn ou npm)
